...
async function runForOrg(orgId: number, jobId: string | undefined): Promise<void> {
  ...
  const cronLog = await prisma.cronLog.create({
    data: { jobName: 'dailyReport', orgId, startedAt: new Date(), status: 'running' },
  })

  try {
    // @ts-expect-error — legacy JS engine has no type declarations (Phase 1.5 migration)
    const fn = (await import('../../../../src/engines/dailyReport.js')).default as () => Promise<unknown>
    await fn()
    ...
    logger.info({ orgId, jobId }, 'dailyReport completed')
  } catch (err) {
    await prisma.cronLog.update({
      where: { id: cronLog.id },
    ...
    await prisma.errorLog.create({
      data: { orgId, source: 'worker:dailyReport', errorMessage: String(err), stackTrace: (err as Error).stack ?? null },
    })
    throw err
  }
}

export const dailyReportWorker = new Worker<JobData>(
  'dailyReport',
