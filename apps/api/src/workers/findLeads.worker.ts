...
async function runForOrg(orgId: number, jobId: string | undefined): Promise<void> {
  ...
  try {
    // The legacy JS engine is now multi-tenant aware: it accepts orgId as its
    // first parameter and wraps its body in runWithOrg(orgId, …) so every
    // Prisma read/write inside resolves to the requesting org's scoped client.
    // Plan limits (claudeDailySpendCapUsd, geminiQueriesPerDay, leadsPerDay)
    // are still read from process.env — wiring them through `sub.plan.limitsJson`
    // is tracked separately.
    // @ts-expect-error — legacy JS engine has no type declarations
    const findLeads = (await import('../../../../src/engines/findLeads.js')).default as (orgId: number) => Promise<unknown>
    await findLeads(orgId)
