...
async function runForOrg(orgId: number, jobId: string | undefined): Promise<void> {
  ...
  const cronLog = await prisma.cronLog.create({
    data: { jobName: 'sendEmails', orgId, startedAt: new Date(), status: 'running' },
  })

  try {
    // @ts-expect-error — legacy JS engine has no type declarations
    const fn = (await import('../../../../src/engines/sendEmails.js')).default as (orgId: number) => Promise<unknown>
    await fn(orgId)
    ...
    logger.info({ orgId, jobId }, 'sendEmails completed')
  } catch (err) {
    await prisma.cronLog.update({
      where: { id: cronLog.id },
    ...
    await prisma.errorLog.create({
      data: { orgId, source: 'worker:sendEmails', errorMessage: String(err), stackTrace: (err as Error).stack ?? null },
    })
    throw err
  }
}

export const sendEmailsWorker = new Worker<JobData>(
  'sendEmails',
